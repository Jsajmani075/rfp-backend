...
const getairecomendationHandler = async (request) => {
  ...
  const proposals = await db.Proposal.findAll({
    where: { rpfId: id },
    attributes: ["id", "vendor_id", "emailBodyContent"]
  });
  ...
  STRICT RANKING RULE (IMPORTANT):
- Ranking MUST be assigned purely based on the score.
- Rank 1 = highest score; Rank 2 = next best; and so on.
- NO TIES allowed. NO duplicate ranks.
- Even if two proposals are similar, AI MUST give different scores and different ranks.
- Ranks must always be 1, 2, 3, ... in perfect order.
- If there is only one proposal, it must still receive a score and rank = 1.

  
  Instructions:
  1. Read ALL vendor proposals carefully.
  2. Evaluate each separately based on the criteria above.
  3. Assign a score from 1–10 (higher = better).
  4. Rank all proposals strictly by score (rank 1 = best).
